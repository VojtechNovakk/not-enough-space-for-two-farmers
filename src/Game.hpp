...
private:
    ...
    Menu m_menu;
    short int m_winner = 0;

    void startGame();
    void endGame(short int winner);
    void processEvent();
...
public:
    ...
    m_awayFarm(std::make_shared<Farm>(false, sf::Vector2u(width, height))), m_menu({m_window.getSize().x, m_window.getSize().y}) {};
    void run();
};